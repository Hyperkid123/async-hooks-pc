...
import { RemoteHookContext } from './RemoteHookProvider';

type UseRemoteHookResult<T> = {
  id: string;
  loading: boolean;
  error: Error | null;
  hookResult?: T;
...
const useRemoteHook = <T>({
  ...
  useEffect(() => {
    if (id) {
      ...
      const hasChanged = args.length !== prevArgs.length ||
